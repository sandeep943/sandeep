...
<div class="container" ng-app="myApp" ng-controller="MyController" ng-init="getDataFromServer()">
  ...
  <table class="table table-bordered table-striped">
    ...
    <tbody>
      <tr ng-repeat="f in forum | orderBy:sortType:sortReverse | filter:searchFtitle">
           		<td>{{f.forumid}}</td>
           		<td>{{f.forumtitle}}</td>
           		<td>{{f.forumcontent}}</td>
           		<td>{{f.forumUserName}}</td>
           		<td>{{f.creationdatetime}}</td>
           		<td>{{f.category}}</td>
           		<td><a href="<c:url value="/ForumView?forumid={{f.forumid}}"/>">View Forum
												</a></td>
      </tr>
    </tbody>
